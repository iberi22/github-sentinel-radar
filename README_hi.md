# 🛡️ गिटहब संतरी और रडार (GitHub Sentinel & Radar)

<div align="center">

[![English](https://img.shields.io/badge/Language-English-blue.svg)](README.md)
[![Español](https://img.shields.io/badge/Idioma-Espa%C3%B1ol-green.svg)](README_es.md)
[![中文](https://img.shields.io/badge/%E8%AF%AD%E8%A8%80-%E4%B8%AD%E6%96%87-red.svg)](README_zh.md)
[![हिन्दी](https://img.shields.io/badge/%E0%A4%AD%E0%A4%BE%E0%A4%B7%E0%A4%BE-%E0%A4%B9%E0%A4%BF%E0%A4%A8%E0%A5%8D%E0%A4%A6%E0%A5%80-orange.svg)](README_hi.md)
[![Français](https://img.shields.io/badge/Langue-Fran%C3%A7ais-yellow.svg)](README_fr.md)
[![العربية](https://img.shields.io/badge/%D8%A7%D9%84%D9%84%D8%BA%D8%A9-%D8%A7%D9%84%D8%B9%D8%B1%D8%A8%D9%8A%D8%A9-emerald.svg)](README_ar.md)
[![বাংলা](https://img.shields.io/badge/%E0%A6%AD%E0%A6%BE%E0%A6%B7%E0%A6%BE-%E0%A6%AC%E0%A6%BE%E0%A6%82%E0%A6%B2%E0%A6%BE-purple.svg)](README_bn.md)
[![Português](https://img.shields.io/badge/Idioma-Portugu%C3%AAs-teal.svg)](README_pt.md)
[![Русский](https://img.shields.io/badge/%D0%AF%D0%B7%D1%8B%D0%BA-%D0%A0%D1%83%D1%81%D1%81%D0%BA%D0%B8%D0%B9-cyan.svg)](README_ru.md)
[![اردو](https://img.shields.io/badge/%D8%B2%D8%A8%D8%A7%D9%86-%D8%A7%D8%B1%D8%AF%D9%88-pink.svg)](README_ur.md)

</div>

---


> **ऑन-डिमांड तकनीकी बुद्धिमत्ता फ़ीड, एंटी-बॉट रक्षा कवच और सत्यापित ओपन-सोर्स डेवलपर्स का नेटवर्क।**
> सरल और मॉड्यूलर परतों के साथ निर्मित, जो 100% मुफ़्त गिटहब एक्शन और गिटहब पेजेस पर संचालित होता है।

---

## 🌟 प्रमुख विशेषताएँ

1. **🛡️ एंटी-बॉट संतरी**: फर्जी फॉलोअर्स और बैकडेटेड कमिट वाले बॉट्स को स्वचालित रूप से पहचानता और ब्लॉक करता है।
2. **📡 ऑन-डिमांड टेक रडार**: आपके पसंदीदा रिपॉजिटरी से नवीनतम रिलीज़ और आपके स्टैक से मेल खाते नए प्रोजेक्ट ढूँढता है।
3. **⚡ संसाधन बचत और सुप्त मोड**: भारी खोज केवल आपकी मांग पर चलती है। 7 दिनों तक उपयोग न होने पर यह स्वचालित रूप से रुक जाता है।
4. **🌐 बॉर्डरलेस और बहुभाषी इंटरफ़ेस**: बिना किसी बॉर्डर के आधुनिक स्वच्छ डिज़ाइन, डार्क/लाइट मोड और दुनिया की शीर्ष 10 भाषाओं में अनुवाद।
5. **🤝 सत्यापित डेवलपर्स नेटवर्क**: वास्तविक रचनाकारों को जोड़ने वाला एक विकेंद्रीकृत गिट-आधारित डेटाबेस।
6. **🔍 समीक्षा कतार (मानव निर्णय)**: फॉलोअर्स और फॉलोइंग को ट्रस्ट % के साथ विश्वसनीय/संभावित बॉट सूचियों में बाँटता है। कुछ भी ऑटो-ब्लॉक नहीं: चुनें, कॉपी करें और वर्कफ़्लो से ब्लॉक करें। ब्लॉक तभी निष्पादित होते हैं जब आप स्वतः-निर्मित ब्लॉक-रिक्वेस्ट PR को मर्ज करते हैं।

---

## 🚀 2 मिनट में सेटअप करें

1. **Use this template** से अपना सार्वजनिक रिपॉज़िटरी बनाएँ।
2. कहे जाने पर Actions सक्षम करें। Settings → Pages में **GitHub Actions** चुनें और **Deploy GitHub Pages** एक बार चलाएँ।
3. डैशबोर्ड में अपडेट दबाएँ, GitHub पर **Run workflow** की पुष्टि करें और लौटें। **ब्राउज़र टोकन या रडार का अतिरिक्त सीक्रेट नहीं चाहिए।**
4. व्यक्तिगत ब्लॉकिंग वैकल्पिक है: [Sentinel सेटअप](DEPLOYMENT.md#sentinel)।

## 📜 लाइसेंस
MIT License © 2026 by [iberi22](https://github.com/iberi22) और समुदाय।

> Deployment / permisos / validation: [DEPLOYMENT.md](DEPLOYMENT.md) is the audited setup reference (2026-09-07).
